 ...
 *
 *
 */var _require2 = require('jest-message-util'), formatStackTrace = _require2.formatStackTrace,
    separateMessageFromStack = _require2.separateMessageFromStack;
var _require3 =


        require('jest-matcher-utils'), RECEIVED_BG = _require3.RECEIVED_BG, RECEIVED_COLOR = _require3.RECEIVED_COLOR,
    getType = _require3.getType, highlightTrailingWhitespace = _require3.highlightTrailingWhitespace,
    matcherHint = _require3.matcherHint, printExpected = _require3.printExpected,
...
var toThrowMatchingStringOrRegexp = function toThrowMatchingStringOrRegexp(name,
                                                                           error,
                                                                           pattern,
                                                                           value) {
...
var toThrowMatchingErrorInstance = function toThrowMatchingErrorInstance(name,
                                                                         error,
                                                                         expectedError) {
    if (error && !error.message && !error.name) {
        error = new Error(error);
...
var toThrowMatchingError = function toThrowMatchingError(name,
                                                         error,
                                                         ErrorClass) {
...
var printActualErrorMessage = function printActualErrorMessage(error) {
    if (error) {
        var _separateMessageFromS =
                separateMessageFromStack(error.stack), message = _separateMessageFromS.message,
